...
static uint64_t nvme_rq_in;
static uint64_t nvme_rq_out;
...
int nvme_mpath_alloc_disk(struct nvme_ctrl *ctrl, struct nvme_ns_head *head)
{
	...
	nvme_rq_in = ktime_get_real_fast_ns();
	if (!head->disk)
...
void nvme_mpath_remove_disk(struct nvme_ns_head *head)
{
	...
	nvme_rq_out = ktime_get_real_fast_ns();
	printk("Latency for nvme pdu %d is: %lluns:",++pdu_count,(nvme_rq_out-nvme_rq_in));
	put_disk(head->disk);
}
